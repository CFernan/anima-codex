...
import { z } from "zod";
import { AllCategoryDefinitionSchema } from "$lib/schema/category";
import { CombatInvestmentSchema } from "$lib/schema/combat";
import { SecondaryInvestmentSchema } from "$lib/schema/secondary";
...
const VALID_COMBAT_KEYS = new Set(Object.keys(CombatInvestmentSchema.shape));

const VALID_SECONDARY_GROUPS: Record<string, Set<string>> = Object.fromEntries(
  Object.entries(SecondaryInvestmentSchema.shape).map(([group, schema]) => [
    group,
    new Set(
...
// ---------------------------------------------------------------------------
// Parse once — shared across all tests in this file
// ---------------------------------------------------------------------------
...
describe("defaultCategories — structural invariants", () => {
  ...
  it("no category has duplicate arquetipos", () => {
    ...
    for (const [name, cat] of Object.entries(parsed.data)) {
      ...
      expect(unique.size, `"${name}" has duplicate arquetipos`).toBe(cat.arquetipos.length);
    }
  });
